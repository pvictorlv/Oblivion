...
namespace Oblivion.HabboHotel.Items.Wired.Handlers.Conditions
{
    internal class FurniHasUsers : IWiredItem
    {
        ...
        public bool Execute(params object[] stuff) => !Items.Any() || Items.Where(current => current != null && Room.GetRoomItemHandler().FloorItems.Contains(current)) .Where(current => !current.AffectedTiles.Values.Any(current2 => Room.GetGameMap().SquareHasUsers(current2.X, current2.Y))).All(current => Room.GetGameMap().SquareHasUsers(current.X, current.Y));
    }
}