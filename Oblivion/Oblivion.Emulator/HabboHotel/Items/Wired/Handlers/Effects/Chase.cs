...
namespace Oblivion.HabboHotel.Items.Wired.Handlers.Effects
{
  internal class Chase : IWiredItem, IWiredCycler
  {
    ...
    public async Task<bool> OnCycle()
    {
      ...
      {
        if (item == null || !Room.GetRoomItemHandler().FloorItems.ContainsKey(item.Id))
        {
        ...
        if (Point == null)
        {
          ...
          Point = HandleMovement(item.Coordinate, movement, item.Rot);
        }

        if (Point.Value.X == item.X && Point.Value.Y == item.Y)
          continue;

        if (Room.GetGameMap().ItemCanMove(item, Point.Value))
        {
          var NewZ = Room.GetGameMap().SqAbsoluteHeight(Point.Value.X, Point.Value.Y);
          ...
          if (Point != item.Coordinate)
          {
            ...
            await serverMessage.AppendIntegerAsync(Point.Value.X);
            await serverMessage.AppendIntegerAsync(Point.Value.Y);
            ...
            await Room.GetRoomItemHandler().SetFloorItem(item, Point.Value.X, Point.Value.Y, NewZ);
          }
        }
