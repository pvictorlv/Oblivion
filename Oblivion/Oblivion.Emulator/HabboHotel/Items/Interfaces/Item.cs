...
namespace Oblivion.HabboHotel.Items.Interfaces
{
    ...
    internal class Item
    {
        ...
        {
            using (var queryReacter = Oblivion.GetDatabaseManager().GetQueryReactor())
            {
                queryReacter.SetQuery(
                    "UPDATE LOW_PRIORITY furniture SET stack_height = @height, can_stack = @stack, allow_trade = @trade, interaction_modes_count = @modes WHERE id = " +
                    id);
