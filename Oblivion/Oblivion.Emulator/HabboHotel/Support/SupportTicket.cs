...
namespace Oblivion.HabboHotel.Support
{
    ...
    internal class SupportTicket
    {
            ...
            string message, uint roomId, string roomName, double timestamp, List<string> reportedChats)
        {
            ...
            RoomName = roomName;
            Timestamp = timestamp;
            _senderName = Oblivion.GetGame().GetClientManager().GetNameById(senderId);
            _reportedName = Oblivion.GetGame().GetClientManager().GetNameById(reportedId);
            _modName = Oblivion.GetGame().GetClientManager().GetNameById(ModeratorId);
