...
namespace Oblivion.HabboHotel.Pets
{
    ...
    internal class Pet
    {
        ...
        internal Room Room => !IsInRoom ? null : Oblivion.GetGame().GetRoomManager().GetRoom(RoomId);
        ...
        internal int Age
        {
            get
            {
                var creation = Oblivion.UnixToDateTime(CreationStamp);
        ...
        internal string OwnerName => Oblivion.GetGame().GetClientManager().GetNameById(OwnerId);
        ...
        {
            {
                ...
                if (num <= 4)
                    num = 15;
                var randomNumber = Oblivion.GetRandomNumber(4, num);
                if (!add)
                {
                    Energy -= randomNumber;
                    if (Energy < 0)
